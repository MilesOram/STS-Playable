...
public:
    theGuardian(int mS = 30, int cMS = 30, bool dM = false, bool fAD = false, void (theGuardian::* dI)() = nullptr) :
        modeShift(mS), currentModeShift(cMS), defMode(dM), firstAttackDone(fAD), declaredIntent(dI), nonpc(240, "The Guardian") {
        enemyIDNumber = 16; allAttacks = { "chargingUp","fierceBash","ventSteam","whirlwind","roll","twinSlam","defensiveMode" };
    }
    ~theGuardian() {}
    void offensiveMode();
    void chargingUp();
...
public:
    hexaGhost(int dD = 0, int aI = 0, bool fAD = false, void (hexaGhost::* dI)() = nullptr) :
        dividerDamage(dD), attackIndex(aI), firstAttackDone(fAD), declaredIntent(dI), nonpc(250, "HexaGhost") {
        enemyIDNumber = 17; allAttacks = { "sear","tackle","activate","divider","inflame","inferno" };
    }
    ~hexaGhost() {}
    void activate();
    void divider();
